...
class SMCAnalyzer:
        ...
        if not fvg.empty:
            for idx, row in fvg.iterrows():
                gap_info = {
                    ...
                    "direction": row.get('FVG', 0),
                    "mitigated_index": row.get('MitigatedIndex', None)
                }
    ...
    def calculate_bos_choch(self, df: pd.DataFrame, swing_highs_lows: pd.DataFrame, close_break: bool = True) -> Dict[str, Any]:
        """
    ...
    def calculate_order_blocks(self, df: pd.DataFrame, swing_highs_lows: pd.DataFrame, close_mitigation: bool = False) -> Dict[str, Any]:
        """
    ...
    def calculate_liquidity(self, df: pd.DataFrame, swing_highs_lows: pd.DataFrame, range_percent: float = 0.01) -> Dict[str, Any]:
        """
